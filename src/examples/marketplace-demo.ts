...
import { Effect, Layer, Schedule, Duration, pipe, Array as EffectArray, Exit, Fiber } from 'effect'
...
export type VendorEvent = VendorRegistered | VendorActivated | VendorSuspended | VendorProfileUpdated
...
export type VendorCommand = RegisterVendor | ActivateVendor | SuspendVendor | UpdateVendorProfile
...
export class VendorDomainService {
  static create(id: VendorId, data: RegisterVendor['data']): Effect.Effect<VendorAggregate> {
  ...
  static activate(vendor: VendorAggregate): Effect.Effect<VendorAggregate> {
    return vendor.status === VendorStatus.PENDING
      ? Effect.succeed({ ...vendor, status: VendorStatus.ACTIVE, version: vendor.version + 1 })
      : Effect.fail(new Error(`Cannot activate vendor in ${vendor.status} status`))
  }

  static suspend(vendor: VendorAggregate, reason: string): Effect.Effect<VendorAggregate> {
    return vendor.status === VendorStatus.ACTIVE
      ? Effect.succeed({ ...vendor, status: VendorStatus.SUSPENDED, version: vendor.version + 1 })
      : Effect.fail(new Error(`Cannot suspend vendor in ${vendor.status} status`))
  }
...
export class ProductDomainService {
  static create(id: ProductId, data: CreateProduct['data']): Effect.Effect<ProductAggregate> {
    return Effect.succeed({
      id,
  ...
  static activate(product: ProductAggregate): Effect.Effect<ProductAggregate> {
    return product.status === ProductStatus.DRAFT
      ? Effect.succeed({ ...product, status: ProductStatus.ACTIVE, version: product.version + 1 })
      : Effect.fail(new Error(`Cannot activate product in ${product.status} status`))
  }
}
...
export class OrderDomainService {
  static create(id: OrderId, data: CreateOrder['data']): Effect.Effect<OrderAggregate> {
  ...
  static confirm(order: OrderAggregate, paymentId: PaymentId): Effect.Effect<OrderAggregate> {
    return order.status === OrderStatus.PENDING
      ...
      : Effect.fail(new Error(`Cannot confirm order in ${order.status} status`))
  }
}
...
export class PaymentDomainService {
  static initiate(id: PaymentId, data: InitiatePayment['data']): Effect.Effect<PaymentAggregate> {
    return Effect.succeed({
      id,
  ...
  static complete(payment: PaymentAggregate, transactionId: string, fees: Money): Effect.Effect<PaymentAggregate> {
    return payment.status === PaymentStatus.PENDING
      ...
      : Effect.fail(new Error(`Cannot complete payment in ${payment.status} status`))
  }
}
...
const registerVendorHandler = createCommandHandler({
  ...
  execute: (cmd) => Effect.gen(function* () {
    ...
    const vendor = yield* VendorDomainService.create(cmd.aggregateId, cmd.data)
...
const orderFulfillmentSaga = createEventHandler({
  ...
  handle: (event) => Effect.gen(function* () {
    ...
    yield* logger.info(`🚀 Order fulfillment saga started for order: ${event.aggregateId}`)
    yield* logger.info(`📦 Processing ${event.data.items.length} items`)
    yield* logger.info(`💰 Total amount: ${event.data.totalAmount.amount} ${event.data.totalAmount.currency}`)
    
    // Simulate async processing
    yield* Effect.sleep(Duration.millis(500))
    yield* logger.info(`✅ Order fulfillment saga completed for: ${event.aggregateId}`)
  })
})

const paymentCompletionSaga = createEventHandler({
  canHandle: (event): event is PaymentCompleted => event.type === 'PaymentCompleted',
  handle: (event) => Effect.gen(function* () {
    const logger = yield* LoggerService
    
    yield* logger.info(`💳 Payment completion saga started for payment: ${event.aggregateId}`)
    yield* logger.info(`✅ Payment completed successfully at: ${event.data.completedAt}`)
  })
})
...
const vendorListProjection = createProjection({
  name: 'VendorList',
  handlers: {
    VendorRegistered: (event) => Effect.gen(function* () {
      const cache = yield* CacheService
      const logger = yield* LoggerService
      ...
      yield* cache.set(`vendor-list:${event.aggregateId}`, JSON.stringify(vendorItem))
      yield* logger.info(`Updated vendor list projection for: ${event.aggregateId}`)
    }),
    
    VendorActivated: (event) => Effect.gen(function* () {
      const cache = yield* CacheService
      const logger = yield* LoggerService
      
      const existingData = yield* cache.get(`vendor-list:${event.aggregateId}`)
      if (existingData) {
        const vendor: VendorListItem = JSON.parse(existingData)
        const updatedVendor = { ...vendor, status: VendorStatus.ACTIVE }
        yield* cache.set(`vendor-list:${event.aggregateId}`, JSON.stringify(updatedVendor))
        yield* logger.info(`Activated vendor in projection: ${event.aggregateId}`)
      }
    })
  }
})
...
const productCatalogProjection = createProjection({
  name: 'ProductCatalog',
  handlers: {
    ProductCreated: (event) => Effect.gen(function* () {
      const cache = yield* CacheService
      const logger = yield* LoggerService
      ...
      yield* cache.set(`product-catalog:${event.aggregateId}`, JSON.stringify(productItem))
      yield* logger.info(`Updated product catalog projection for: ${event.aggregateId}`)
    }),
    
    ProductActivated: (event) => Effect.gen(function* () {
      const cache = yield* CacheService
      const logger = yield* LoggerService
      
      const existingData = yield* cache.get(`product-catalog:${event.aggregateId}`)
      if (existingData) {
        const product: ProductCatalogItem = JSON.parse(existingData)
        const updatedProduct = { ...product, status: ProductStatus.ACTIVE }
        yield* cache.set(`product-catalog:${event.aggregateId}`, JSON.stringify(updatedProduct))
        yield* logger.info(`Activated product in catalog: ${event.aggregateId}`)
      }
    })
  }
})
...
const orderHistoryProjection = createProjection({
  name: 'OrderHistory',
  handlers: {
    OrderCreated: (event) => Effect.gen(function* () {
      const cache = yield* CacheService
      const logger = yield* LoggerService
      ...
      yield* cache.set(`order-history:${event.aggregateId}`, JSON.stringify(orderItem))
      yield* logger.info(`Updated order history projection for: ${event.aggregateId}`)
    }),
    
    OrderConfirmed: (event) => Effect.gen(function* () {
      const cache = yield* CacheService
      
      const existingData = yield* cache.get(`order-history:${event.aggregateId}`)
      if (existingData) {
        const order: OrderHistoryItem = JSON.parse(existingData)
        const updatedOrder = { ...order, status: OrderStatus.CONFIRMED }
        yield* cache.set(`order-history:${event.aggregateId}`, JSON.stringify(updatedOrder))
      }
    })
  }
})
...
// Health Checks
const createHealthChecks = () => Effect.gen(function* () {
  const logger = yield* LoggerService
  ...
  const checks = {
    eventStore: Effect.gen(function* () {
      ...
      Effect.catchAll(() => Effect.succeed({ status: 'unhealthy', service: 'event-store' }))
    ),
    ...
    cache: Effect.gen(function* () {
      ...
      Effect.catchAll(() => Effect.succeed({ status: 'unhealthy', service: 'cache' }))
    )
  }
  
  return {
    runHealthChecks: () => Effect.gen(function* () {
      const results = yield* Effect.all(checks)
      const overallHealth = Object.values(results).every(r => r.status === 'healthy') 
        ? 'healthy' 
        : 'unhealthy'
      
      yield* logger.info(`Health check completed: ${overallHealth}`, { results })
...
const MockEventStoreLive = Layer.succeed(
  ...
  EventStoreService.of({
    ...
    saveEvents: (aggregateId, events, expectedVersion) => Effect.succeed(undefined),
...
// Main Application
const runMarketplaceDemo = () => Effect.gen(function* () {
  const logger = yield* LoggerService
  ...
  try {
    ...
    return Exit.fail(error instanceof Error ? error : new Error(String(error)))
  }
})
...
if (import.meta.main) {
  const program = pipe(
    runMarketplaceDemoWithResilience,
    Effect.tapBoth({
      onFailure: (error) => Effect.logError('Marketplace demo failed', { error }),
      onSuccess: (result) => Effect.logInfo('Marketplace demo succeeded', { result })
    })
  )
  ...
  Effect.runPromise(program)
    .then(() => {
      ...
      console.log('🎯 Demonstrated: Multiple domains, CQRS/ES, Effect-TS, sagas, projections')
      process.exit(0)
    })
    .catch((error) => {
