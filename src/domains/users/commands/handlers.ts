...
export function registerUserCommandHandlersWithPattern(
  ...
  registerCommandPattern<UserCommand, ICommandResult>(commandBus, {
    [UserCommandTypes.ChangeUserPassword]: async (command: Commands.ChangeUserPasswordCommand) => {
      ...
      return makeCommandSuccess({ userId: command.aggregateId }, { executionTime: Date.now(), version: aggregate.version });
    },
