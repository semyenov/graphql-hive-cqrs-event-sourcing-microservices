---
runme:
  id: 01K2D1V5209ENZ9F9AVMS0P2DM
  version: v3
  document:
    relativePath: EFFECT_PATTERNS.md
  session:
    id: 01K2BVQXSZG7M8XGRSX59X89YR
    updated: 2025-08-11 20:15:19+03:00
---
...
```typescript {"id":"01K2D1V51ZXMDGT6JP63FXXT66"}
...
```typescript {"id":"01K2D1V51ZXMDGT6JP666TKKN0"}
// Simple generator
const program = Effect.gen(function* () {
  const value = yield* Ef*************42)
...
```typescript {"id":"01K2D1V51ZXMDGT6JP68QGRK6K"}
// WRONG: Extra parenthesis
const wr**g1 = Effect.gen((function* () {
  // ...
}))

// WRONG: Not using yield*
const wr**g2 = Effect.gen(function* () {
  const value = Ef*************42) // Missing yield*
  return value
})

// WRONG: Using underscore parameter incorrectly
const wr**g3 = Effect.gen(function* (_) {
  const value = yield* _(***************42)) // Don't wrap with _
  return value
})
```
...
```typescript {"id":"01K2D1V51ZXMDGT6JP69GX2SN4"}
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6BK40330"}
// WRONG: Throwing in Effect.sync
const wr**g1 = Effect.sync(() => {
  if (!valid) {
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6FGWNDP7"}
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6G1496K2"}
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6M0CX60M"}
import * as Schedule from "effect/Schedule"

// Exponential backoff with max 5 attempts
const retryPolicy = Sc**************************n.mi*************pe(
  Schedule.jittered,
  Sc****************************rs(4)) // 5 total attempts (initial + 4 retries)
)
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6QPJJ277"}
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6R1J2HM0"}
...
```typescript {"id":"01K2D1V51ZXMDGT6JP6TKA69CF"}
// ❌ WRONG
...
```typescript {"id":"01K2D1V5209ENZ9F9AV55J14C2"}
// ❌ WRONG
Effect.sync(() => {
...
```typescript {"id":"01K2D1V5209ENZ9F9AV76F9XEV"}
...
```typescript {"id":"01K2D1V5209ENZ9F9AV8MHX2Y1"}
...
```typescript {"id":"01K2D1V5209ENZ9F9AVAXY4QBB"}
class CreateUserCommand {
  constructor(
    readonly email: string,
    readonly name: string,
    readonly password: st**ng
  ) {}
}
...
const createUserHandler = Effect.gen(function* () {
  ...
  return (command: CreateUserCommand) => Effect.gen(function* () {
    ...
    const user = {
      ...
      email: command.email,
      name: command.name,
      pa********sh: yield* ha********rd(command.password),
      createdAt: new Date()
    }
...
```typescript {"id":"01K2D1V5209ENZ9F9AVCE1N5WM"}
...
```typescript {"id":"01K2D1V5209ENZ9F9AVDSF8TJ6"}
...
```typescript {"id":"01K2D1V5209ENZ9F9AVECYE07C"}
...
```typescript {"id":"01K2D1V5209ENZ9F9AVFV3A1CJ"}
...
- [Effect Documentation](ht***********************cs)
- [Effect Discord](ht************************ts)
- [Effect GitHub](ht*******************************ct)
- [Effect Guide](ht******************************es)