// Universal branded types for CQRS/Event Sourcing framework
// These types provide compile-time guarantees and prevent mixing incompatible values

// Generic brand type
export type Brand<T, TBrand extends string> = T & { readonly __brand: TBrand };

// Framework Core ID Types - universal across all domains
...
// Universal type constructors with validation
export const BrandedTypes = {
  // Framework Core ID constructors
  ...
  // Framework Timestamp constructors
  ...
  // Framework Version constructors
  ...
  // Generic String format constructors
...
export const BrandedTypeGuards = {
  isAggregateId: (value: unknown): value is AggregateId => {
    return typeof value === 'string' && value.length > 0;
  },

  isEventId: (value: unknown): value is EventId => {
    return typeof value === 'string' && value.length > 0;
  },
