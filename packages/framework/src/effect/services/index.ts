...
export interface EventStoreService<TEvent extends IEvent = IEvent> {
  readonly getEvents: (
    ...
    ReadonlyArray<Extract<TEvent, { type: TEvent["type"] }>>,
    Error,
    never
  >;
  ...
  readonly saveEvents: (
    aggregateId: AggregateId,
    events: ReadonlyArray<Extract<TEvent, { type: TEvent["type"] }>>,
  ...
  readonly subscribe: (
    callback: (event: Extract<TEvent, { type: TEvent["type"] }>) => void,
  ) => Effect.Effect<() => void, never, never>;
}

export const EventStoreService = Context.GenericTag<EventStoreService>(
  "EventStoreService",
);
...
export const CommandBusService = Context.GenericTag<CommandBusService>(
  "CommandBusService",
);
...
export const QueryBusService = Context.GenericTag<QueryBusService>(
  "QueryBusService",
);
...
export const ProjectionService = Context.GenericTag<ProjectionService>(
  "ProjectionService",
);
...
export const MetricsService = Context.GenericTag<MetricsService>(
  "MetricsService",
);
...
export interface ConfigurationService {
  ...
  readonly getDuration: (
    key: string,
  ) => Effect.Effect<Duration.Duration, Error, never>;
}
...
export const LoggerServiceLive = Layer.succeed(
  ...
  LoggerService.of({
    ...
    error: (message, error, context) =>
      Effect.sync(() => console.error(message, error, context)),
  }),
);
...
export const MetricsServiceLive = Layer.succeed(
  ...
  MetricsService.of({
    increment: (name, value = 1, tags) =>
      Effect.sync(() =>
        console.log(`Metric increment: ${name}`, { value, tags })
      ),
    ...
    get: <T>(key: string) =>
      Config.string(key).pipe(
        ...
        Effect.mapError((error) =>
          new Error(`Config error for ${key}: ${error}`)
        ),
      ),
...
export function adaptLegacyServices<TEvent extends IEvent>(
  ...
  EventStoreService | CommandBusService | QueryBusService,
  never,
  never
> {
  const eventStoreLayer = Layer.succeed(
    ...
    EventStoreService.of({
      getEvents: (aggregateId, fromVersion) =>
        Effect.tryPromise({
          try: () =>
            implementations.eventStore.getEvents(aggregateId, fromVersion),
      ...
      saveEvents: (aggregateId, events, expectedVersion) =>
        Effect.tryPromise({
          try: async () => {
            await implementations.eventStore.appendBatch(
              events.map((event) =>
  ...
  const queryBusLayer = Layer.succeed(
    QueryBusService,
    QueryBusService.of({
      execute: <TResult>(query: IQuery) =>
        Effect.tryPromise({
          try: () => implementations.queryBus.ask(query) as Promise<TResult>,
