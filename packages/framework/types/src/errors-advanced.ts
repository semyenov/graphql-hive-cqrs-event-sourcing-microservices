// Comprehensive error type hierarchy with discriminated unions
...
export const Result = {
  ...
  err: <E extends AppError>(error: E): Result<never, E> => ({ success: false, error }),
  
  isOk: <T, E extends AppError>(result: Result<T, E>): result is { success: true; value: T } => 
    result.success,
  
  isErr: <T, E extends AppError>(result: Result<T, E>): result is { success: false; error: E } => 
    !result.success,
  
  map: <T, U, E extends AppError>(
    result: Result<T, E>,
    fn: (value: T) => U
  ): Result<U, E> => {
    if (result.success) {
